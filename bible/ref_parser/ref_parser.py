...
RE_BOOK = re.compile(r"""
    ((\d[\s_\-])?[А-Я][а-я]+)\.?\s*                       # book short title
    (,\s*(\d+|\d+.\d+)\s*зач\.\s*(\([^\)]+\))?)?,?\s*     # text before chapter
    ([IVXL\d]+)[^А-Яа-я\.\<\>\(\)]+[.;,]?                 # chapter with lines
""", re.VERBOSE)
...
class RefBook:
    ...
    Book short title may start with digit.
    And must follow the defined title format: \d Name
    ...
    def __init__(self, book, chapters=None):
        self.book = book.replace("_", " ").replace("-", " ")
    ...
    def __parse_lines(self, chapter, lines, range_start=False, range_end=False):
        ...
        all lines greater than this line should be taken.
